...
Este diretório transforma o template vazio no sistema financeiro operante do titular. Quem
executa é o agente — papel, modos e regras de conduta estão em [AGENTS.md](../AGENTS.md), que
vence em qualquer conflito. Quem decide é o titular, sempre.

São sete fases, F0–F6, cada uma com arquivo próprio, gate de saída verificável e valor entregue
mesmo que o onboarding pare ali. **Cada fase entrega valor sozinha e destrava a seguinte** — é
desenho, não acidente: quem para na F3 já sai com mandato, fontes organizadas e orçamento, e
nada do que foi feito depende do que ainda não foi.
...
| [F1 — mandato](f1-mandato.md) | `plan/mandato.md` v1: número-alvo, risco, tetos, IPS, cadências | entrevista de 10 blocos | 45–60 min |
| [F2 — fontes e extratos](f2-fontes-e-extratos.md) | `fontes/<fonte>/` com README + `raw/` para cada fonte declarada | exportar extratos (12 meses ou o que houver) | 30–60 min + espera dos exports |
| [F3 — orçamento e recorrentes](f3-orcamento-e-recorrentes.md) | `plan/orcamento.csv`, `recorrentes.csv`, `categorias.csv` + 1º fluxo de caixa | entrevista de renda e fixos; validar metas | 45–60 min |
| [F4 — patrimônio](f4-patrimonio.md) | `plan/patrimonio.csv` (snapshot datado) + patrimônio líquido | saldos que o raw não cobre | 30–45 min |
...
| [F6 — rotina e governança](f6-rotina-e-governanca.md) | `plan/rotina-mensal.md`, `plan/riscos.md`, 1º comitê agendado | escolher datas; validar riscos | 30–45 min |

Total típico: 4–6 horas de conversa, em 2–5 sessões (ver "Sessões" abaixo).
...
- **Pendência registrada conta como gate cumprido** quando o arquivo da fase disser (ex.: a F2
  fecha com fonte que tem dir + README + pendência datada, mesmo sem raw). O que não pode existir
  é buraco silencioso: ou o artefato está lá, ou a pendência está em `plan/proximos-passos.md`.
...
Valem todas as regras de conduta de [AGENTS.md](../AGENTS.md); na entrevista, as que mais pesam:
...
- **"Não sei" e "depois" são respostas válidas.** Viram pendência com data e valor em jogo em
  `plan/proximos-passos.md`, e a entrevista segue. Nada trava o onboarding esperando resposta.
...
- **Explique o porquê de cada bloco antes de perguntar.** O titular responde melhor sabendo para
  que serve a resposta — os arquivos de fase já trazem o racional pronto de cada pergunta.
- **Anuncie o progresso dentro da fase.** "Bloco 4 de 10 do mandato" custa uma linha e dá ao
  titular a noção exata de quanto falta.

## Retomada

O onboarding é retomável por construção: qualquer sessão nova detecta a fase pelo estado do repo
e continua do ponto exato. Resumo da detecção — **o primeiro artefato ausente define a fase**:
...
| — (tudo existe) | operação |

A tabela canônica — sinais completos, modos e regras de uso — está em
...
3. Releia o que já está registrado na fase e NUNCA re-pergunte o que já tem resposta gravada —
   re-perguntar ensina ao titular que o registro não vale nada.

## Sessões

O onboarding completo leva **2–5 sessões**. A pausa natural é o fim de fase: gate fechado,
commit feito, valor entregue — o titular sai com algo pronto mesmo se demorar a voltar.

- Sugira a pausa você mesmo ao fechar um gate ("bom ponto para parar; na próxima sessão
  começamos a F4"). Sessão longa demais degrada a qualidade das respostas de entrevista.
- A F1 costuma merecer uma sessão inteira: é a conversa mais importante do sistema.
