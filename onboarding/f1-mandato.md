...
[../templates/mandato.md](../templates/mandato.md), status "v1: provisório até o 1º snapshot
de patrimônio (F4)".
**Gate para F2:** mandato commitado e confirmado pelo titular após leitura de volta.
**Pré-requisito:** F0 fechada, com o checklist de saída de [f0-preparacao.md](f0-preparacao.md)
...
- **Uma pergunta por vez.** Registre a resposta no rascunho do mandato antes da próxima
  pergunta. Racional: bloco de perguntas gera resposta rasa; o registro imediato preserva a
...
- **"Não sei / depois" nunca trava.** Registre um default provisório, marcado como provisório,
  e abra pendência em `plan/proximos-passos.md` com o que muda conforme a resposta.
- **O agente propõe, o titular decide.** Rascunhos e sugestões são do agente; o número final é
  do titular.
- Duração típica: 45 a 60 min. Se o titular cansar, pause no fim de um bloco. A entrevista é
...
F4 (patrimônio atual) darão a base para propor um número no 1º comitê (ex.: patrimônio
...
em poucos anos sem risco desproporcional; promessa nesse sentido não é confiável.

**Exemplos de resposta:** "assalariado com renda crescente; o motor é a carreira e a carteira
protege e compõe"; "vendi minha participação, vivo da carteira; o papel é renda com
preservação".
...
conservador, independente do que declare. Sem histórico, o perfil é **não testado**
...
estabilidade da renda: CLT estável, 3 a 6 meses; renda variável, PJ ou autônomo, 6 a 12. O
custo fixo dado aqui é aproximado de propósito: a F3 o refina com dados reais e o agente
atualiza o mandato se a diferença for relevante.

**Exemplos de resposta:** "custo fixo ~R$ 8.000/mês; 6 meses viram reserva de R$ 48.000 em
...
usa os tetos como limite em toda recomendação futura, sem precisar rediscutir. Veto é
...
ajustar aos blocos 3 a 7) e marque como provisório:
...
**Por quê:** o aporte mínimo é a variável mais controlável do plano: mais que retorno, mais
que alocação. Melhor um mínimo defensável cumprido todo mês que um valor ideal descumprido.
Definir o que conta evita distorção do registro nos dois sentidos: amortizar dívida cara é
alocação de capital (não despesa), e se contar como aporte, a comparação amortizar × investir
entra como avaliação recorrente da rotina, não como decisão pontual.
...
**Por quê:** sistema sem cadência deixa de ser usado em três meses. Cada camada tem função
distinta: **rotina mensal** (30 a 45 min) é operação (classificar, dar baixa, republicar);
**comitê trimestral** é decisão (desvios de banda, riscos, análises, ata em
`plan/decisions.md`); **revisão anual** é o único momento em que o mandato muda fora de
evento de vida. O dia da rotina veio da F0 (sugestão: dia 5, que fecha o mês anterior antes
do bloco de vencimentos do dia 10).
...
1. **Preencher `plan/mandato.md`** a partir de
   ...
   Primeira linha de status: **"v1: provisório até o 1º snapshot de patrimônio (F4)"**.
   Racional: o mandato só deixa de ser provisório quando a F4 confrontar a política com o
   patrimônio real.
2. **Ler de volta:** resuma o mandato ao titular em 5 a 8 linhas (número, motor, reserva,
   tetos, regra de dívida, alvo de alocação, aporte, cadências) e peça confirmação explícita.
...
- [ ] `plan/mandato.md` com status "v1: provisório até o 1º snapshot de patrimônio (F4)"
...
pedido de extratos. Boa pausa natural: o mandato fecha uma sessão com valor entregue.
