...
Tirar o primeiro balanço completo: uma linha por item, ativos e passivos, tudo na mesma data.
Este é o número que o resto do sistema acompanha — a aba Patrimônio (F5), o drift contra o IPS,
o comitê trimestral (F6). Sem snapshot honesto, a política de alocação do mandato é decorativa.

Princípios que regem a fase:

- **Snapshot é datado e append-only.** `plan/patrimonio.csv` acumula linhas por data: a rotina
  mensal (F6) acrescenta um snapshot novo, nunca edita um antigo. Racional: a série histórica é
  o que permite ver evolução e drift; editar o passado destrói as duas coisas.
- **Célula sem fonte é célula vazia + pendência.** Nunca estimar um saldo "de cabeça" para
  fechar o balanço. Um número inventado hoje vira decisão errada no comitê. O que faltar entra
  em `plan/proximos-passos.md` com data-limite.
...
| bens | valor conservador; só itens acima do corte (sugestão: R$ 5.000) | NF ou estimativa declarada | inventariar miudeza custa mais do que informa |
| previdencia | valor de resgate na data | app/extrato da seguradora | é o que existe de fato, não a projeção |
...
`plan/recorrentes.csv`, F3); o que reduz o patrimônio é o estoque da dívida. Registrar o valor
...
de empresa própria é subjetivo, infla o PL e não é vendável amanhã. Se o titular quiser esse
número, é análise formal (F6), não linha de balanço.

**Moeda-base sempre.** `patrimonio.csv` não tem coluna de moeda: todo valor entra na moeda-base
de `plan/perfil.md`. Item em moeda estrangeira converte pelo câmbio da data do snapshot e
registra o câmbio usado em `obs`. Racional: balanço em moeda mista não soma.
...
2. O que o dado bruto não cobre (previdência sem export, imóvel, veículo, bem, saldo que o
   titular informa de viva voz) entra em `plan/saldos-manuais.csv` com **fonte e `data_ref`**
...
substitua as linhas de exemplo por dados reais — exemplo fictício nunca fica no repo do titular.
...
2. **Compare com o IPS do mandato** (`plan/mandato.md`, bloco 8). Percentuais por classe são
   calculados sobre os **ativos brutos** (soma dos positivos), não sobre o PL — banda de alocação
   diz onde o capital está; dívida é assunto das regras de alavancagem do mandato (bloco 7).
   Se o snapshot é mais granular que o IPS, agregue as classes na comparação.
...
4. **Desvio fora da banda vira pauta do 1º comitê — NUNCA rebalanceamento automático.** Registre
   cada desvio como item numerado em `plan/proximos-passos.md` ("pauta do 1º comitê: …"). O
   agente propõe, o titular decide, e decide no comitê (F6) — decisão patrimonial em cima de um
   snapshot recém-nascido, sem análise, é exatamente o improviso que o sistema existe pra evitar.
5. **Se o IPS era provisório** (marcado assim na F1 porque o titular não sabia a alocação-alvo),
   este é o momento de recalibrar: apresente a alocação real e pergunte, bloco a bloco, se o alvo
   proposto continua fazendo sentido. Ajuste aceito vira registro em `plan/decisions.md`
   (decisão, racional, dados no momento).
6. **Remova a marca "provisório" — sempre, ao fim da comparação.** Vale para os dois caminhos:
   IPS recalibrado (item 5) ou IPS que o titular já tinha definido na F1 — neste caso, confirme
   com ele que os alvos seguem valendo diante da alocação real e registre a confirmação em
   `plan/decisions.md`. Atualize a linha de status de `plan/mandato.md` de
   ...
   mandato sem a marca — mandato eternamente provisório não governa nada.
...
- [ ] Status do mandato atualizado — marca "provisório até 1º snapshot" removida após a
      comparação (recalibrada ou confirmada pelo titular)
- [ ] Commit: `f4: snapshot de patrimônio`

Próxima fase: [f5-planilha-e-agenda.md](f5-planilha-e-agenda.md) — publicar os espelhos.
